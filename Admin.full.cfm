...
<!---
<title>[<CFOUTPUT>#CGI.SERVER_NAME#</CFOUTPUT>] Law Department Contingent Liabilities: System Admin</title>
--->


<title>[<CFOUTPUT>#cgi.local_addr#</CFOUTPUT>] Law Department Contingent Liabilities: System Admin</title>
...
<!---
div.columns       { width: 850px; padding:10pt}

div.columns div   { width: 400px; height: 100px; float: left; }

--->


div.columns       { width: 100%; padding:10pt}
...
<cfif cgi.SERVER_NAME eq "127.0.0.1">
	<cfset RespondingUser_Id = "K6GVN0">
<cfelseif cgi.SERVER_NAME eq Dev_Server_Name>
	<cfset RespondingUser_Id = "YSRJ00">
<cfelseif isdefined("cgi.SERVER_NAME") and cgi.SERVER_NAME eq Dev_Server_Name and cgi.SERVER_PORT eq "8550" and isdefined("cgi.AUTH_USER") and len(cgi.AUTH_USER) eq 0>
	<cfset RespondingUser_Id = "K6GVN0">
<cfelseif isdefined("cgi.SERVER_NAME") and cgi.SERVER_NAME eq Dev_Server_Name and cgi.SERVER_PORT eq "8550" and isdefined("cgi.AUTH_USER") and len(cgi.AUTH_USER) eq 0>
	<cfset RespondingUser_Id = "YSRJ00">
<cfelseif isdefined("cookie._MFA.NAMEID_CLRSADMIN")>
	<cfset RespondingUser_id = cookie._MFA.NAMEID_CLRSADMIN>
<cfelse>
	<cfset RespondingUser_Id = TRIM(UCASE(RemoveChars(cgi.auth_user,1,find('\',cgi.auth_user))))>
</cfif>

<!---KS comment out 1.22.26 incorrect setup --->

<!---<cfif cgi.SERVER_NAME eq "56.76.100.176">
	<cfset RespondingUser_Id = "K6GVN0">
<cfelseif cgi.SERVER_NAME eq "eagnmnbd240">
	<cfset RespondingUser_Id = "YSRJ00">
<cfelseif isdefined("cgi.SERVER_NAME") and cgi.SERVER_NAME eq "eagnmnbd240" and cgi.SERVER_PORT eq "8550" and isdefined("cgi.AUTH_USER") and len(cgi.AUTH_USER) eq 0>
	<cfset RespondingUser_Id = "K6GVN0">
<cfelseif isdefined("cgi.SERVER_NAME") and cgi.SERVER_NAME eq "eagnmnbd240" and cgi.SERVER_PORT eq "8550" and isdefined("cgi.AUTH_USER") and len(cgi.AUTH_USER) eq 0>
	<cfset RespondingUser_Id = "YSRJ00">
<cfelseif isdefined("cookie._mfa.nameid_clrsadmin")>
...
AND 
LatestReportDate LT "12/1/#LatestReportDate_CalYear#">
...
<table width="85%" border="0" style="padding-bottom:0; margin-bottom:0">
...
<CFIF IsLocalDev>
<a href="../V1.0/Report.cfm" target="_blank" style="font-weight:bold">Contingent Liabilities and Receivables</a>
<CFELSE>
<a href="../../ClientService/ContingentLiabilities/V1.0/Report.cfm" target="_blank" style="font-weight:bold">Contingent Liabilities and Receivables</a>
</CFIF>

<tr>

<td style="padding:10pt; padding-bottom:0" >
...
<div style="color:white; font-weight:normal; font-style:normal; margin-top:0">
...
<!---
<iframe id="ThisIFrame" src="setTestUser.cfm" width="85%" height="50pt" frameborder="0" scrolling="no" marginwidth="0" marginheight="0" style="padding:3pt">
</iframe>
--->



<div style="padding:10pt; padding-left:20pt; padding-bottom:15pt; margin-bottom:20pt; background-color:white">

<li><a href="setTestUser.cfm" target="_blank"><b>Set Test User</b></a>
...
<!---
<div style="border:thin solid bfdfff; padding:10pt; padding-top:5pt; padding-bottom:0pt; margin-bottom:20pt">
--->
...
<div style="border:thin solid bfdfff; padding:10pt; padding-top:5pt; padding-bottom:0pt; height:90pt; margin-bottom:20pt">
...
<li><a href="RecordsList.CurrentQuarter.cfm"><b>Case Records List for Current Quarter</b></a>
...
<li><a href="Documentation/ContingentLiabilitiesOutline.docx"><b>Contingent Liabilities System Outline</b></a>
...
<CFIF Get_Latest_Update.RecordCount GT 0>
	...
	<a href="#URL_Scheme##This_Server##V1_URL_Path#Report.cfm###Get_Latest_Update.PRIMARYKEY#" target="_blank">#Get_Latest_Update.CASE_NAME#</a>
...
<p>
<div style="border:thin solid orange; padding:5pt; margin-bottom:10pt; margin-top:20pt">
<!---
For Current Report:
--->
...
<CFIF EndOfYearFlag NEQ "EOY">
	...
	<p>
	<li><b>Email to All Offices</b></a> 	&nbsp;
	
	<i>[Sender on "From" line:
	...
	<form name="SendEmailForm_Latest" METHOD="POST" action="SendReportForUpdate.cfm">
...
<!--- Value set in SaveSelectFile.cfm  (#CFFILE_RFS_Uploads_Dir_Link##name#) --->

	<input type="hidden" name="GCRequest_FileName">
...
--->
    ...
    <li><b>General Counsel's memo requesting updates</b>
...
--->
	...
	<CFSET ThisSaveFileIFrameHeight = 250>
	
	
	<div id="IFrameSaveFile" style="position:relative; background:tan; margin-top:5pt; width:75%">

<!---
SaveSelectFile.cfm?CaseRecIDSeq=#CONTINGENT_LIAB_GetRecord.CASE_REC_ID_SEQUENCE#&RecIDParm=#RecIDParm#
--->
	
	<CFOUTPUT>
	<iframe id="ThisSaveFileIFrame" name="ThisSaveFileIFrame"  src="SaveSelectFile.cfm" width="100%" height="#ThisSaveFileIFrameHeight#" frameborder="0" scrolling="no" marginwidth="0" marginheight="0">
	</iframe>
	</cfoutput>
	...
	<br>
	<textarea id="IntroNote" NAME="IntroNote" ROWS="6" COLS="75" style="font-family:arial; margin-bottom:15pt"><CFOUTPUT>#This_Intro_Note#</CFOUTPUT></textarea>
	

	<CFOUTPUT>
	<li><a href="" onClick="SendEmailForm_Latest.Sender.value = '#RespondingUser_Id#'; checkDeadlineDate(SendEmailForm_Latest); return false"><b>Send Draft_1</b></a> <i>(messages go only to user clicking this link)</i>
	</cfoutput>
	
	<p>
	<li><a href="" onClick="SendEmailForm_Latest.Sender.value = ''; checkDeadlineDate(SendEmailForm_Latest); return false"><b>Send to All Offices</b></a>
...
<CFELSE>
	...
	<p>
	<li><a href="" onClick="SendEmailForm_EOY.Sender.value = ''; SendEmailForm_EOY.submit(); return false"><b>Send to All Offices</b></a>
...
<p>
<li><a href="" onClick="EAJA_SendEmailForm_Latest.Sender.value = ''; checkDeadlineDate(EAJA_SendEmailForm_Latest); return false"><b>Send to All Offices</b></a>
...
<CFIF CONTINGENT_LIAB_GetRecord_NextReportDate.RecordCount EQ 0>
	<li><a href="LoadRecords.NextQuarter.cfm"><b>Load Case Records</b></a><i>   (Do Not clicking this link. Only Whitehead, Nia have potential updates and review final case lists report)</i>
<CFELSE>
...
<CFIF EndOfYearFlag NEQ "EOY">
	...
	<li><a href="" onClick="LoadRecordsEOY.submit(); return false"><b>Load Case Records</b></a><i>   (Do Not clicking this link. Only Whitehead, Nia have updates and review final End-of-Year Report)</i></li>
	</ol>
